...
%load("T_MinNorma_UAV_DinInv_Real.mat");
...
%% Colorbar
scatter(h(1, :),h(2, :),30,error_dynamic,'filled','MarkerFaceAlpha',0.7);
c = colorbar;
...
%% Colorbar
scatter(h(1, :),h(3, :),30,error_dynamic,'filled','MarkerFaceAlpha',0.7);
c = colorbar;
c.Label.Interpreter = 'latex';
c.Label.String = '$\textrm{Error}~||\tilde{\mathbf{\eta}}||$';
...
index_zoom_1 = (t_mpc>=20) & (t_mpc<=30);
error_x_plot_zoom = line(t_mpc(index_zoom_1),hd(1,(index_zoom_1))-h(1,(index_zoom_1)));
error_y_plot_zoom = line(t_mpc(index_zoom_1),hd(2,(index_zoom_1))-h(2,(index_zoom_1)));
error_z_plot_zoom = line(t_mpc(index_zoom_1),hd(3,(index_zoom_1))-h(3,(index_zoom_1)));
...
%% Data generation
error_vx_plot = line(t_mpc(1,1:length(mu(1,:))),mu(1,:));
set(error_vx_plot, 'LineStyle', '-', 'Color', C7, 'LineWidth', 1.3*lw);
error_vy_plot = line(t_mpc(1,1:length(mu(2,:))),mu(2,:));
set(error_vy_plot, 'LineStyle', '-', 'Color', C14, 'LineWidth', 1.3*lw);
error_vz_plot = line(t_mpc(1,1:length(mu(3,:))),mu(3,:));
