...
[hxd, hyd, hzd, hpsid, hxdp, hydp, hzdp, hpsidp] = Trayectorias(3,t,6);
...
for k=1:length(t)-N
    ...
    %%
    tic
    [u_opt,x_opt] = SolverUAV_MPC_din(h,nu,hd,N,x_N,v_N,args,solver,k);
    sample_MPC(k)=toc;
    ...
    %% DYNAMIC ESTIMATION
    [Test(:,k),chi_estimados(:,k+1)] = estimadaptive_dymanic_UAV(chi_estimados(:,k),uc_p(:,k), uc(:,k), nu(:,k), hd(1:4,k), h(:,k) ,A,B, L, ts);
    mu(:,k)= uc(:,k)+1*Test(:,k);
    ...
    x(:,k+1) = UAV_Dinamica_RK4_T(chi_uav,x(:,k),mu(:,k),L,ts,Tu(:,k));
...
%%
save("Adaptive_MPC.mat","t","sample_MPC","hd","h","nu","mu","uc", "Tu", "Test");
...
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperPosition', [0 0 10 4]);
plot(t(1:length(sample_MPC)),sample_MPC,'Color',[46,188,89]/255,'linewidth',1); hold on
grid on;
